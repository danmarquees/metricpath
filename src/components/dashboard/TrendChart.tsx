...
export const TrendChart = ({ volume = 12500 }: TrendChartProps) => {
    // Generate a visual trend based on base volume
    const baseK = volume / 1000;
    const dataPts = [
        baseK * 0.4, baseK * 0.6, baseK * 0.45, baseK * 0.7, 
        baseK * 0.85, baseK * 0.6, baseK * 0.95, baseK, 
        baseK * 0.8, baseK * 0.75, baseK * 0.9, baseK * 1.1
    ].map(v => Math.round(v));
    
    // Find max for scaling
    const maxVal = Math.max(...dataPts) || 1;
